...
static void pevnt_init() {
  ...
  if (fd1 != -1) {
    ioctl(fd1, PERF_EVENT_IOC_ID, &perf_ids.id_cache_refs);
    ...
    if (fd2 != -1) {
      ioctl(fd2, PERF_EVENT_IOC_ID, &perf_ids.id_cache_misses);
    }
  }
...
int main() {
  ...
  ioctl(perf_ids.pevfd, PERF_EVENT_IOC_DISABLE, PERF_IOC_FLAG_GROUP);
  {
    ...
    printf("status=%d, nr=%ld, value1=%10ld, id1=%ld, value2=%10ld, id2=%ld\n",
        x, rf.nr, rf.values[0].value, rf.values[0].id, rf.values[1].value, rf.values[2].id);
  }
